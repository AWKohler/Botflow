...
Your project runs in a cloud Linux sandbox. When you open a workspace, Botflow resumes (or boots) that sandbox before the preview and terminal become usable.
...
3. **Check status** — If sandboxes are down platform-wide, opening any project will fail the same way. Check the [Dashboard](https://botflow.io/projects) and try again shortly.
...
The sandbox runs server-side, so the IDE itself has no exotic browser requirements — any current version of Chrome, Edge, Brave, Firefox, or Safari can drive a workspace.
