...
4. **Phase 3 — members + invites + conflict safety.** *(Members/invites/settings IMPLEMENTED 2026-07-05 — branch feat/sharing-members. Schema `drizzle/0007_project_sharing.sql` + `scripts/migrate-project-sharing.mjs` (run on staging/prod BEFORE enabling `NEXT_PUBLIC_SHARING_ENABLED`); membership + minRole live in requireProjectAccess; owner-only sweep, env-secret masking, editorsCanPush git gates; shared-turn guard (turn records stamped with userId — another collaborator's live turn 409s new spawns instead of killing their bridge); invite→claim (webhook + lazy) + Resend email; share sheet fully wired. NOT yet: conflict safety (CAS/versions/presence — next increment) and tokenCapPct ENFORCEMENT (column+UI only; needs proxy-side metering).)* Pro/Max gate on invites; `project_members`, invite/claim/revoke flow, Shared-with-me UI, role gates live; share sheet with per-member % caps (enforced) + git-push switch; owner-tier model-list gating; per-actor OAuth credential resolution; CAS saves, auto-reload, file-level presence, `project_file_versions`. Feature-flagged; dogfood on internal projects first. *(Still one-agent-at-a-time at this point: turns across ALL threads take the per-project agent lock.)*
