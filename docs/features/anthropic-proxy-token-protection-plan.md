...
- **Billing identity.** When user B takes a turn in user A's shared project, whose credential pays?
  ...
  - **RESOLVED 2026-07-02 — hybrid** (see `project-sharing-plan.md` §5.1): Claude/Codex **OAuth turns always use the acting user's own connected account** — personal OAuth tokens are never shared across users; the owner's OAuth credential is injected only for the owner's own turns. **Platform-metered model turns bill the owner's credit pool**, capped per-collaborator by a share-sheet % slider enforced at this proxy/metering layer. Model availability follows the owner's tier. Exception: when `SHARING_ALLOW_OWNER_OAUTH=1` (platform env kill switch) AND the project's owner-set share-sheet switch is on, a collaborator's OAuth turn MAY resolve to the owner's account (billing = owner) — TOS-compliance escape hatch, default off everywhere, enforced here at credential resolution.
