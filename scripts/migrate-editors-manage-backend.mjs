// Sharing: editors_manage_backend switch.
// Usage: node scripts/migrate-editors-manage-backend.mjs
...
async function run() {
  ...
  const migrationPath = path.resolve(process.cwd(), 'drizzle/0010_editors_manage_backend.sql');
  ...
  console.log(`Running ${statements.length} statements from 0010_editors_manage_backend.sql...`);
  ...
  const cols = await sql.query(
    `SELECT column_name, data_type FROM information_schema.columns
     WHERE table_name = 'projects' AND column_name = 'editors_manage_backend' ORDER BY ordinal_position`,
  );
  console.log('\neditors_manage_backend column:');
