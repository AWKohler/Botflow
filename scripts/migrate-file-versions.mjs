// Conflict safety: project_file_versions table.
// Usage: node scripts/migrate-file-versions.mjs
...
async function run() {
  ...
  const migrationPath = path.resolve(process.cwd(), 'drizzle/0008_file_versions.sql');
  ...
  console.log(`Running ${statements.length} statements from 0008_file_versions.sql...`);
  ...
  const cols = await sql.query(
    `SELECT column_name, data_type FROM information_schema.columns
     WHERE table_name = 'project_file_versions' ORDER BY ordinal_position`,
  );
  console.log('\nproject_file_versions columns:');
