// Sharing: share_owner_credits switch.
// Usage: node scripts/migrate-share-owner-credits.mjs
...
async function run() {
  ...
  const migrationPath = path.resolve(process.cwd(), 'drizzle/0009_share_owner_credits.sql');
  ...
  console.log(`Running ${statements.length} statements from 0009_share_owner_credits.sql...`);
  ...
  const cols = await sql.query(
    `SELECT column_name, data_type FROM information_schema.columns
     WHERE table_name = 'projects' AND column_name = 'share_owner_credits' ORDER BY ordinal_position`,
  );
  console.log('\nshare_owner_credits column:');
