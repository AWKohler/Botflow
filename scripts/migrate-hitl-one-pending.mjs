// HITL "one pending request per project" migration.
// Usage: node scripts/migrate-hitl-one-pending.mjs
...
//
// Applies drizzle/0004_hitl_one_pending.sql: dedupes extra pending rows and adds
// partial unique indexes (project_id WHERE status='pending') on the three
// request tables.
...
async function run() {
  ...
  const migrationPath = path.resolve(process.cwd(), 'drizzle/0004_hitl_one_pending.sql');
  ...
  console.log(`Running ${statements.length} statements from 0004_hitl_one_pending.sql...`);
