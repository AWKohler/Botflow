...
export type RateLimitBucket =
  ...
  | 'agent' | 'claudeCode' | 'toolCallback' | 'anthropicProxy'
...
export const RATE_LIMIT_BUCKETS: Record<RateLimitBucket, { tokens: number; window: Duration }> = {
  ...
  anthropicProxy:{ tokens: envInt('RL_ANTHROPIC_PROXY', 120), window: '60 s' }, // /api/internal/anthropic-proxy (one call per agent-loop step; keyed by binding userId)
