...
// Platform-managed read-only files the agent must not edit (mirrors the Stripe
// write-guard in sandboxed-web-tools.ts). Each is regenerated by the platform,
// so edits are pointless — and a rewrite could weaken signature verification or
// hardcode a key.
const READONLY_PATHS = new Map<string, string>([
  [
    "/convex/revenueCatWebhook.ts",
    "it verifies the RevenueCat webhook signature and is re-written on connect — your edit would be lost and could break payment verification. Put your logic in convex/revenueCatBilling.ts instead.",
  ],
  [
    "/Sources/Core/RevenueCatConfig.swift",
    "it carries the RevenueCat public SDK key + routing prefix and is regenerated before every build — your edit would be lost. Read RevenueCatConfig.apiKey / .namespacedAppUserId() from it instead.",
  ],
]);
function readOnlyBlock(path: string): { ok: false; error: string; path: string } | null {
  const normalized = path.startsWith("/") ? path : `/${path}`;
  const reason = READONLY_PATHS.get(normalized);
  if (reason) {
    return {
      ok: false,
      error: `${normalized} is auto-generated by Botflow (${reason})`,
      path: normalized,
    };
  }
  return null;
  ...
  const baseTools = {
    ...
    write: tool({
      ...
      async execute({ path, content }) {
        const blocked = readOnlyBlock(path);
    ...
    askQuestion: tool({
      ...
      async execute({ questions }, ctx) {
        return safe(async () => {
          ...
          // Poll until the user answers. 270s (< the agent route's 300s
          // maxDuration) so the timeout-dismiss below runs before the platform
          // kills the request.
          ...
          // Timed out — best-effort mark dismissed so future UI doesn't show
          // stale. Guard on status='pending' so a just-answered question can't be
          // clobbered back to dismissed.
          await db
      ...
      ? {
          initializeRevenueCatPayments: tool({
            description:
              ...
              "IMPORTANT: Real purchases require the user to finish Apple-side setup (paid Apple Developer account, Paid Apps agreement + banking/tax, products created and PASSED App Review, distribution via TestFlight/App Store). Completing a purchase cannot be demoed on the streamed simulator (`.storekit` files don't apply to simctl launches) — testing purchases needs a sandbox tester on TestFlight/device. Always set this expectation with the user.\n\n" +
              "ROUTING (do this when wiring the RevenueCat SDK): the platform writes `Sources/Core/RevenueCatConfig.swift` (public SDK key + this project's App User ID prefix) before every build — never edit or hardcode it. Configure with `Purchases.configure(withAPIKey: RevenueCatConfig.apiKey, appUserID: RevenueCatConfig.namespacedAppUserId(<your app's user id>))` (e.g. the Convex auth user id), guarded by `RevenueCatConfig.isConfigured` since the key is empty until the user finishes connecting. The prefix routes entitlement webhooks to this project so they never leak to the user's other apps; it is also returned in this tool's result under `routing.appUserIdPrefix`. If you can't set a stable user id yet, RevenueCat's anonymous id still works (the platform falls back to broadcasting), but namespacing is strongly preferred.",
            inputSchema: z.object({}),
            async execute() {
              ...
              try {
                ...
                // Hand the agent the exact namespaced App User ID prefix so it
                // configures RevenueCat to route webhooks to THIS project.
                return {
                  ...parsed,
                  routing: {
                    appUserIdPrefix: namespacedAppUserIdPrefix(projectId),
                    instruction:
                      "Set RevenueCat appUserID = this prefix + your app's user id, so entitlement webhooks route to this project only.",
                  },
                };
              } catch {
                return {
