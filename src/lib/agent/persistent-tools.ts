...
// write-guard in sandboxed-web-tools.ts). The RevenueCat webhook receiver
// verifies the platform's signature; an agent rewrite could weaken it. It's
// re-scaffolded on (re)connect, so edits are pointless anyway.
const READONLY_PATHS = new Set(["/convex/revenueCatWebhook.ts"]);
function readOnlyBlock(path: string): { ok: false; error: string; path: string } | null {
  const normalized = path.startsWith("/") ? path : `/${path}`;
  if (READONLY_PATHS.has(normalized)) {
    return {
      ok: false,
      error: `${normalized} is auto-generated by Botflow (it verifies the RevenueCat webhook signature) and is re-written on connect — your edit would be lost and could break payment verification. Put your logic in convex/revenueCatBilling.ts instead.`,
      path: normalized,
    };
  }
  return null;
      ...
      ? {
          initializeRevenueCatPayments: tool({
            description:
              ...
              "ROUTING (do this when wiring the RevenueCat SDK): set the RevenueCat App User ID with THIS project's namespace so entitlement webhooks route to this project and never leak to the user's other apps. Configure Purchases with appUserID = \"bfp_<projectId>__\" + <your app's user id> (e.g. the Convex auth user id). The exact prefix is returned in this tool's result under `routing.appUserIdPrefix`. If you can't set a stable user id yet, RevenueCat's anonymous id still works (the platform falls back to broadcasting), but namespacing is strongly preferred.",
            inputSchema: z.object({}),
            async execute() {
              ...
              try {
                ...
                return {
                  ...parsed,
                  routing: {
                    appUserIdPrefix: `bfp_${projectId}__`,
                    instruction:
