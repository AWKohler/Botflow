    ...
    return [
      ...
      ...(hasBackend
        ? [
            ...
            "- **Function names are unchecked strings** — a stale name fails only at runtime. When repurposing the template demo, delete `Item.swift`/`ItemsStore.swift`/`convex/items.ts` outright; after renaming or removing ANY backend function, `Grep` `Sources/` for the old `\"file:function\"` strings and fix every call site BEFORE deploying.",
