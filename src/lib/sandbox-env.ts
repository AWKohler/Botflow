...
/**
 * Render the generated `RevenueCatConfig.swift` — how the public SDK key
 * (`appl_…`) and the project-namespaced App User ID prefix reach the app. The
 * agent prompt tells the agent to call
 * `Purchases.configure(withAPIKey: RevenueCatConfig.apiKey, appUserID:
 * RevenueCatConfig.namespacedAppUserId(<its user id>))` and never hardcode a
 * key, so this file MUST exist in every Swift project (empty key until the
 * user connects — `isConfigured` lets the app guard).
 */
export function swiftRevenueCatConfigContent(
  ...
  return [
    ...
    `    /// Public Apple SDK key (appl_…). Empty until RevenueCat is connected`,
    `    /// in the Payments tab — gate Purchases.configure on isConfigured.`,
 ...
 * can reference `RevenueCatConfig` before the user finishes connecting: the key
 * is the user's `rc_public_sdk_key` once this project is 'connected', empty
 * otherwise. XcodeGen's `Sources/` glob picks the file up on the remote build.
 */
export async function materializeSwiftRevenueCatConfig(projectId: string): Promise<void> {
  ...
  let publicSdkKey: string | null = null;
  if (project.revenuecatStatus === "connected") {
    ...
    publicSdkKey = identity?.rcPublicSdkKey ?? null;
  }

  await sandboxWriteFile(
    projectId,
    SWIFT_REVENUECAT_CONFIG_PATH,
    swiftRevenueCatConfigContent(publicSdkKey, namespacedAppUserIdPrefix(projectId)),
  );
}

/**
 * All platform-managed Swift config files, in one call. Use this at every
 * build entry point (simulator start/rebuild, device build, App Store submit,
 * convex-deploy reseed) so no path can bake a stale or missing config.
 */
export async function materializeSwiftBuildConfig(projectId: string): Promise<void> {
